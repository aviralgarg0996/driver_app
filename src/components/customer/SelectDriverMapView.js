...
class SelectDriverMapView extends Component<{}> {
  constructor(props) {
    ...
    this.state = {
      ...
      initialPosition:
      {
        latitude: 0,//this.props.locationData.currentLocation.coords.latitude,
        longitude: 0,//this.props.locationData.currentLocation.coords.longitude,
        latitudeDelta: 0.5,
        longitudeDelta: 0.5,
      },
      initialRegion:
  ...
  componentWillReceiveProps(nextProps) {
   ...
   return;  
    ...
    let orderMarker = [];
    let wayPoints = [];
    detailsInfo.orders[0].pickup.map((element, i) => {
      orderMarker.push({
...
return;
  ...
  render() {
    ...
    this.state.orderMarker.map((marker, i) => {
    ...
    return (
      <View style={{ flex: 1 }}>
        <View style={[styles.rootContainer, { height: Constants.BaseStyle.DEVICE_HEIGHT / 100 * height }]}>
          {/* <Text style={{ height: 120, width: 100, position: 'absolute', marginTop: 30, marginLeft: 200, backgroundColor: 'red' }}>
            
          </Text> */}
         {  this.props.locationData.showmap.Orders_Pending 
        ...
        <Modal animationType='slide' transparent={false} visible={this.state.modalVisible}>
          <View style={{ flex: 1, marginTop: scaleHeight(20), backgroundColor: 'transparent' }}>
            ...
            <View style={{ padding: scaleHeight(20) }}>
              ...
              <Image
                resizeMode='cover'
                style={{ width: scaleWidth(100), height: scaleWidth(100), marginLeft: scaleWidth(30), top: scaleHeight(90), position: 'absolute' }}
              ...
              {/* Boxes */}
              <TouchableOpacity>
                <Image style={{ marginLeft: scaleWidth(275), top: scaleHeight(20), position: 'absolute', height: 70, width: 70 }} source={Constants.Images.customer.callCus} />
              </TouchableOpacity>
              <TouchableOpacity>

                <Image style={{ marginLeft: scaleWidth(275), top: scaleHeight(100), position: 'absolute', height: 70, width: 70 }} source={Constants.Images.customer.chatCus} />
              </TouchableOpacity>
              <TouchableOpacity>
                <Image style={{ marginLeft: scaleWidth(275), top: scaleHeight(180), position: 'absolute', height: 70, width: 70 }} source={Constants.Images.customer.noteCus} />
              </TouchableOpacity>
              <TouchableOpacity>
                <Image style={{ marginLeft: scaleWidth(275), top: scaleHeight(260), position: 'absolute', height: 70, width: 70 }} source={Constants.Images.customer.editCus} />
              </TouchableOpacity>
              <TouchableOpacity>

                <Image style={{ marginLeft: scaleWidth(275), top: scaleHeight(340), position: 'absolute', height: 70, width: 70 }} source={Constants.Images.customer.deleteCus} />
              </TouchableOpacity>

              {/* Boxes */}
              <Text style={{ marginLeft: scaleWidth(140), top: scaleHeight(160), position: 'absolute' }}>
                <Text style={styles.review}>45 REVIEWS</Text> <Text style={styles.followers}>50 FOLLOWERS</Text>
              </Text>
              <Text style={{ marginLeft: scaleWidth(140), top: scaleHeight(180), position: 'absolute', flexDirection: 'row', marginTop: scaleHeight(6) }}>
                <Image style={{ height: 15, width: 15 }} source={Constants.Images.customer.reviewStar} />
                <Image style={{ height: 15, width: 15 }} source={Constants.Images.customer.reviewStar} />
                <Image style={{ height: 15, width: 15 }} source={Constants.Images.customer.reviewStar} />
                <Image style={{ height: 15, width: 15 }} source={Constants.Images.customer.reviewStar} />
                <Image style={{ height: 15, width: 15 }} source={Constants.Images.customer.reviewStar} />
...
const styles = StyleSheet.create({
  ...
  titleCss: {
    marginLeft: scaleWidth(140), top: scaleHeight(140), position: 'absolute', fontSize: normalizeFont(14), color: Constants.Colors.DarkBlue, fontWeight: '400'
  },
  review: {
    color: Constants.Colors.SkyBlue, textDecorationLine: 'underline', textDecorationColor: Constants.Colors.SkyBlue, fontSize: normalizeFont(10)
  },
  followers: {
...
export default connect(mapStateToProps, null)(SelectDriverMapView);
