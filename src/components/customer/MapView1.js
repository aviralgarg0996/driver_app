...
class CustomerMapView extends Component {
  constructor(props) {
    ...
    if (this.props.loc[0].address == 'Choose Pickup Location') {
      this.props.state.markerPositions.map(element => {
  ...
  updateDriverLoc = data => {
    ...
    data.driverList.map(item => {
      ...
      if (!driverMap[item._id]) {
        item.onMarkerAction = this.onMarkerAction;
        item.markerData = this.getAvailableDriver(item);
  ...
  componentDidMount() {
    socketUpdate.selectDriverCallback = {
      ...
      socketUpdate.getDriverSelectScreen({
        ...
        lng:
          this.props.locationData.currentLocation != null
            ...
            : 0,
        lat: this.props.state.markerPositions[0]
          ? this.props.state.markerPositions[0].coordinates.latitude
          : this.props.user.orderDetails.orders[0].pickup[0].pickup_point.split(
              ',',
            )[0],
        lng: this.props.state.markerPositions[0]
          ? this.props.state.markerPositions[0].coordinates.longitude
          : this.props.user.orderDetails.orders[0].pickup[0].pickup_point.split(
              ',',
            )[1],
  ...
  checkStatus() {
    ...
    if (this.props.state.Orders.orderId) {
      ...
      if (
        this.props.state.InvoiceData.orderId ||
        this.props.state.Hourly_pickupArr[0].address !== 'Choose Start Location'
      ) {
  ...
  getAvailableDriver = element => {
    ...
    let markerData = {
      coordinates: {
        latitude: element.geometry[0].coordinates[1],
        longitude: element.geometry[0].coordinates[0],
      },
  ...
  getDriverData = () => {
    ...
    if (this.props.state.vehicleID) {
      ...
      if (this.props.state.Orders.vehicleType) {
    ...
    RestClient.getUrlAdmin(
      '/users/getactivedrivers2',
      {
    ...
    return (
      <BoxShadow setting={shadowForItem}>
        <View
          style={[
            styles.container,
            {width: (Constants.BaseStyle.DEVICE_WIDTH / 100) * 76},
          ]}>
                  ...
                  <Text
                    style={[
                      styles.reviewText,
                      {
                        color: '#969297',
                        marginLeft:
                          (Constants.BaseStyle.DEVICE_WIDTH / 100) * 1,
                        marginRight:
                          (Constants.BaseStyle.DEVICE_WIDTH / 100) * 3,
                      },
                    ]}>
                    50 FOLLOWERS
                  </Text>
              ...
              <View
                ...
                <Text style={[styles.orangeText]}>
                  ...
                  <Text
                    style={[
                      styles.reviewText,
                      {color: Constants.Colors.LightGray, fontSize: 14},
                    ]}>
                    {'Orders Delivered'}
                  </Text>
                </Text>
          ...
          <BoxShadow
            ...
            <TouchableOpacity
              ...
              <Text style={[styles.SelectTextStyle]}>{'SELECT DRIVER'}</Text>
            </TouchableOpacity>
          </BoxShadow>
          <Modal
            animationType="slide"
            ...
            <View style={styles.transaparentView}>
              <View style={styles.centerView}>
                ...
                <Text style={styles.topText}>
                  Your Order has not been accepted by drivers.
                </Text>
                <Text style={styles.desc}>
                  1. You can place it in the pool so that it can be seen by
                  drivers.{' '}
                </Text>
                <Text style={styles.desc1}>
                  2. You can select another driver.
                </Text>
                <TouchableOpacity
  ...
  render() {
    ...
    return (
      <View
        ...
        style={{height: height * 0.6, zIndex: 0}}>
        {/* {this.props.locationData.showmap.Home_SelectDriver && */
        true && (
          <MapView
            key={'SelectDriverMapView'}
            ref={ref => {
              this.map = ref;
            }}
...
const styles = StyleSheet.create({
  ...
  OKButtonStyle: {
    ...
    marginRight: (Constants.BaseStyle.DEVICE_WIDTH / 100) * 20,
    borderRadius: 30,
