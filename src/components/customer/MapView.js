...
let ACTIVE_VEHICLE;
let HourlyServiceDate;
let HourlyServiceTime;
...
class CustomerMapView extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      TransportArr: [
        {
          tag: '1',
          displayimg: Constants.Images.customer.bike,
          imgsource: Constants.Images.customer.bike,
          activeimgsource: Constants.Images.customer.active_bike,
          header: 'Bike',
        ...
        {
          tag: '2',
          displayimg: Constants.Images.customer.small,
          imgsource: Constants.Images.customer.small,
          activeimgsource: Constants.Images.customer.active_small,
          header: 'Small',
          backgroundColor: 'transparent',
          borderBottomWidth: 0,
          borderBottomColor: 'transparent',
        ...
        {
          tag: '3',
          displayimg: Constants.Images.customer.medium,
          imgsource: Constants.Images.customer.medium,
          activeimgsource: Constants.Images.customer.active_medium,
          header: 'Medium',
          backgroundColor: 'transparent',
          borderBottomWidth: 0,
          borderBottomColor: 'transparent',
        ...
        {
          tag: '7',
          displayimg: Constants.Images.customer.truck_fridge,
          imgsource: Constants.Images.customer.truck_fridge,
          activeimgsource: Constants.Images.customer.active_truck_fridge,
          header: 'FridgeTruck',
          backgroundColor: 'transparent',
          borderBottomWidth: 0,
          borderBottomColor: 'transparent',
    ...
    setTimeout(() => {
      //    this.SocketAction.callFromPArent();
    }, 4000);
  }
    ...
    if (addFlag) {
      ...
      this.props.dispatch(
        ToastActionsCreators.displayInfo(
          'You can enter max 3 pickup/drop locations.',
        ),
      );
      ...
      this.props.dispatch(
        ToastActionsCreators.displayInfo(
          'You can enter multiple locations either for Pickups or for Drops.',
        ),
      );
      this.props.dispatch({type: 'PLACE_FINDER_MODAL', visibility: false});
    }
      ...
      <View
        ...
        style={[
          {
            ...
            height: (Constants.BaseStyle.DEVICE_HEIGHT / 100) * 9.8,
        ...
        <TouchableOpacity
          ...
          <View
            ...
            <Image
              key={4}
              source={item.displayimg}
              style={[styles.transportIcons, {alignItems: 'center'}]}
              resizeMode={'contain'}
            />
            <Text key={5} style={[styles.transportLabel]}>
              {item.header}
            </Text>
          </View>
        </TouchableOpacity>
  ...
  setDurationTime() {
    this.props.dispatch({
      type: 'SET_DURATION',
      displayDuration: this.state.durationTime[this.state.selectedItem],
      displayItem: [this.state.selectedItem],
    });
  ...
  clickOnEstimate() {
    if (
      this.props.state.pickupArr.length < 2 &&
      this.props.state.HourlyFlag == 0
    ) {
      this.props.dispatch(
        ToastActionsCreators.displayInfo('Please enter Pickup Location..'),
      );
    } else if (
      this.props.state.pickupArr.length < 3 &&
      this.props.state.dropArr.length < 2 &&
      this.props.state.HourlyFlag == 0
    ) {
      this.props.dispatch(
        ToastActionsCreators.displayInfo('Please enter Drop Location..'),
      );
    } else if (
      this.props.state.DeliveryServiceOpacity == 0.8 &&
      this.props.state.HourlyFlag == 0
    ) {
      AdminActions.Get_Admin_Data('/admin/getservice', {}, 'admin').then(
        res => {
      ...
      this.props.state.HourlyServiceCount == 3 &&
      this.props.state.HourlyFlag == 1
    ) {
  ...
  shouldComponentUpdate(currentProp, nextProp) {
    ...
    if (
      currentProp.state.HourlyServiceDate != HourlyServiceDate ||
    ...
    if (
      currentProp.state.pickupArr.length < pickupLength ||
        ...
        <View style={[styles.rootContainer]}>
          {
            ...
            <MapOnly
              onVechileChange={this.props.onVechileChange}
            ...
            style={{
              ...
              bottom:
                this.props.HourlyFlag === 0
                  ? (Constants.BaseStyle.DEVICE_HEIGHT / 100) * 27
                  : (Constants.BaseStyle.DEVICE_HEIGHT / 100) * 27,
          ...
          <View style={[styles.subsubContainer, {opacity: 0.8}]}>
            <FlatList
              data={this.props.state.TransportArr}
              renderItem={({item}) => this.vehicalList(item)}
              horizontal={true}
            />
          </View>
          {this.props.HourlyFlag === 0 ? (
            ...
            <View
              ...
              <View
                style={[
                  styles.ButtonPickupStyle,
                  {
                    opacity: 0.8 /*this.props.state.DeliveryServiceOpacity*/,
                    backgroundColor:
                      '#ffffff' /*this.props.state.EstimateButtonBackgroundColor*/,
                    borderColor:
                      '#ffffff' /*this.props.state.EstimateButtonBackgroundColor*/,
                  },
                ]}>
                {Platform.OS === 'android' ? (
                  <TouchableOpacity onPress={() => this.CallDatePicker()}>
                    <View
                      style={[
                        styles.flexRow,
                        {marginTop: 5, marginBottom: 5, alignItems: 'center'},
                      ]}>
                      <Image
                        source={Constants.Images.customer.calendar}
                        style={[styles.pickupIcons, {flex: 0.2}]}
                        resizeMode={'contain'}
                      />
                      <Text style={styles.HourlyTextStyle}>{'Date'}</Text>
                ...
                {Platform.OS === 'android' ? (
                  <TouchableOpacity onPress={() => this.CallTimePicker()}>
                    <View
                      ...
                      <Image
                        source={Constants.Images.customer.clock}
                        style={[styles.pickupIcons, {flex: 0.2}]}
                        resizeMode={'contain'}
                      />
                      <Text style={styles.HourlyTextStyle}>{'Start Time'}</Text>
                      <Text style={[styles.HourlyRightText]}>
                ...
                <TouchableOpacity
                  ...
                  <View
                    ...
                    <Image
                      source={Constants.Images.customer.duration}
                      style={[styles.pickupIcons, {flex: 0.2}]}
                      resizeMode={'contain'}
                    />
                    <Text style={styles.HourlyTextStyle}>{'Duration'}</Text>
                    <Text style={[styles.HourlyRightText]}>
              ...
              <View
                style={[
                  styles.flexRow,
                  {
                    ...
                    borderBottomWidth: 1,
                    borderBottomColor: Constants.Colors.DarkBlue,
                ...
                <View style={[styles.flexRow, {justifyContent: 'flex-end'}]}>
                  <TouchableOpacity
                    style={[styles.btCloseModal]}
                    onPress={() => {
                      this.setState({isVisible: false});
                    }}>
                    <Image
                      source={Constants.Images.customer.close}
                      style={[styles.btnCloseModalIcon]}
                      resizeMode={'contain'}
                    />
                  </TouchableOpacity>
                </View>
