...
class SearchPlace_Hourly extends Component<{}> {
      ...
      .then((responseData) => {
        if (responseData !== null) {
          ...
          if (this.props.state.PickDropFlag == 1) {
            this.props.dispatch({
              type: 'ADD_PICKUP_HOURLY', pickup: address, lat: arr.result.geometry.location.lat, long: arr.result.geometry.location.lng
            ...
            this.props.dispatch({
              type: 'ADD_DROP_HOURLY', drop: address, lat: arr.result.geometry.location.lat,
  ...
  render() {
    ...
    return (
      <View style={{
        width: '100%', height: "100%", backgroundColor: Constants.Colors.WhiteSmoke,
        ...
        <View style={{ flexDirection: "row", margin: 15 }}>
          <TouchableOpacity style={{ marginleft: 15, marginRight: 30, alignSelf: "center" }}
            onPress={() => { this.props.dispatch({ type: 'PLACE_FINDER_MODAL_HOURLY', visibility: false }) }}>
