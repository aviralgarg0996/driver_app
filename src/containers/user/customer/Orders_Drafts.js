...

        {
          this.state.generalFlag == 1 ?
            ...
            <ScrollView style={[styles.container, { backgroundColor: Constants.Colors.LightBlue }]}>
              <ImageBackground resizeMode='cover' source={require("../../../assets/images/customer/blue.png")} style={[styles.container, { backgroundColor: '#ffffff', marginBottom: scaleHeight(15) }]}>
              ...
              <View>
                <SubmitButton
                  onPress={() => {
                    ...
                    pickupData = this.props.orderDetails.orders[0].pickup[0];


                    tempPickUp = []; //;this.props.state.pickupArr;

                    tempPickUp.push({ "id": 0, "address": pickupData.address, "img": "plus", "next": 3, "prev": 1, "lat": parseFloat(pickupData.pickup_point.split(',')[0]), "long": parseFloat(pickupData.pickup_point.split(',')[1]) });
                    tempPickUp.push(this.props.state.pickupArr[0]);

                    //  alert(JSON.stringify(tempPickUp))

                    this.props.navigation.dispatch({ type: 'ONCHANGE_PICKUP_ORDER', arr: tempPickUp });
                    let routeName = 'Home_Invoice1';
                    if (this.props.orderDetails.deliveryName == "Documents")
                      routeName = 'Home_Invoice1';
                    if (this.props.orderDetails.deliveryName == "Furniture & Appliances")
                      routeName = 'Home_Invoice1';
                    if (this.props.orderDetails.deliveryName == "Courier & Frieght")
                      routeName = 'Home_Invoice1';
