...
class Home extends Component {
  ...
  async componentDidMount() {
    ...
    this.props.ScheduleActions.getWeeklySchedule(
      {...this.state},
    ...
    if (this.props.driverAvailabilityStatus == true) {
      await navigator.geolocation.getCurrentPosition(position => {
        let requestObject = {
          ...
          geometry: {
            coordinates: {
              type: [position.coords.latitude, position.coords.longitude],
            },
          },
          distance: '',
        };
        RestClient.urlAdmin('/drivers/savedriverlatlng', requestObject).then(
          result => {
  ...
  // }
  homeSchedule() {
    this.props.navigation.navigate('Home_ScheduleOrder', {
      selectedDateObj: new Date(),
      refreshCal: this.refreshCalFunction,
    });
  }
  ...
  onDateSelected(date) {
    this.setState(
      {
        selectedStartDate: moment(new Date(date.dateString)).format(
          'YYYY-MM-DD',
        ),
      },
      () => {
        if (
          ...
          this.props.scheduleDatesList[this.state.selectedStartDate].orders !==
            undefined
        ) {
          this.props.navigation.navigate('ManageOrders', {
            selectedDateObj: this.state.selectedStartDate,
          });
        } else {
        ...
        <ScrollView>
          <View style={styles.sectionHeaders}>
            ...
            <TouchableOpacity onPress={() => navigate('Orders')}>
              <Text
                style={[styles.textOrange, {textDecorationLine: 'underline'}]}>
                View all Orders
              </Text>
            </TouchableOpacity>
          </View>
...
const mapStateToProps = state => ({
  tokenforuser:
    (state.user.driverData && state.user.driverData.token) ||
    (state.user.userData && state.user.userData.token),
  modalstate: state.ModalHandleReducer,
  driverAvailabilityStatus: state.user.driverAvailabilityStatus,
  userData:
    (state.user && state.user.driverData) ||
