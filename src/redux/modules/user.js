...
import {setDetails, setCurrentUser} from './location';
...
export const consumerSignup = (data, nav) => {
  ...
  let requestObject = {
    ...
    password:
      data.password && data.password.value && data.password.value.trim(),
  ...
  return dispatch => {
    ...
    RestClient.post('users/', requestObject)
      .then(result => {
        ...
        if (result.status == 1) {
          ...
          if (
            result.data.emailVerified == 0 &&
            result.data.phoneVerified == 0
          ) {
            dispatch(
              CONSUMER_SIGNUP_PHONE_VERICATION({
            ...
            if (result.data.role == 'CUSTOMER') {
              ...
              if (result.data.multiRole[0].status == false) {
                if (result.data.currentstatuss == 'active') {
                  nav();
                  return;
                } else dispatch(DRIVER_FORM_NAVIGATION(result));
              } else {
...
export const phoneVerification = (data, nav) => {
  ...
  return dispatch => {
    dispatch(startLoading());
    RestClient.post('users/phoneOtpMatch', requestObject)
      .then(result => {
        console.log('result signup ******* ', result);
        if (result.status == 1) {
          dispatch(stopLoading());
          ...
          if (result.data && result.data.emailVerified == 0) {
            ...
            if (result.data.role == 'CUSTOMER') {
              ...
              console.log(
                'inside email Verification  done ********* ',
                result.data.phone,
              );
              ch({
...
export const resendPhoneApi = data => {
  ...
  return dispatch => {
    dispatch(startLoading());
    RestClient.post('users/phoneResendRegistration', requestObject)
      .then(result => {
...
export const emailVerification = (data, nav) => {
  ...
  return dispatch => {
    dispatch(startLoading());
    RestClient.post('users/emailOtpMatch', requestObject)
      .then(result => {
        ...
        if (result.status == 1) {
          ...
          if (result.data.phoneVerified == 0) {
            console.log(
              'inside phone Verification not done ********* ',
              result.data.phone,
            );
            dispatch(
              CONSUMER_SIGNUP_PHONE_VERICATION({
...
export const resendEmailApi = data => {
  ...
  return dispatch => {
    dispatch(startLoading());
    RestClient.post('users/emailResendRegistration', requestObject)
      .then(result => {
        //console.log('result email verify ******* ',result)
        if (result.status == 1) {
          dispatch(stopLoading());
...
export const userLogin = (data, nav, navigationObj) => {
  ...
  return dispatch => {
    dispatch(startLoading());
    RestClient.post('users/login', requestObject)
      .then(result => {
        ...
        if (result.status == 1) {
          ...
          AsyncStorage.setItem(
            'first_name',
            result.data && result.data.firstName,
          );
          AsyncStorage.setItem(
            'last_name',
            result.data && result.data.lastName,
          );
          ...
          if (
            ...
            if (result.data.role == 'CUSTOMER') {
              ...
              if (result.data.multiRole[0].status == false) {
                ...
                else dispatch(NAVIGATE_DRIVER_FORM(result));
              } else {
                if (result.data.driverStatus == 'pending') {
                  dispatch({
                    type: 'FORMSUBMIT_VISIBILITY',
  ...
  return dispatch => {
    return new Promise(function(fulfill, reject) {
      ...
      AsyncStorage.getItem('token').then(tokenValue => {
        ...
        dispatch(startLoading());
        RestClient.imageUpload('users/saveuserinfo', body, tokenValue)
          .then(result => {
            ...
            if (result.status == 1) {
              dispatch(stopLoading());
              dispatch(
                ToastActionsCreators.displayInfo('Data saved successfully'),
              );
              fulfill(result);
              // dispatch(ToastActionsCreators.displayInfo(result.message));
            } else {
  ...
  return dispatch => {
    return new Promise(function(fulfill, reject) {
      ...
      AsyncStorage.getItem('token').then(tokenValue => {
        ...
        console.log('tokenvalue---->', tokenValue);
        dispatch(startLoading());
        RestClient.imageUpload('users/profile', body, tokenValue)
          .then(result => {
            console.log('result profile ******* ', result);
...
export const userDriverSecondForm = (data, saveState, onSubmitform) => {
  ...
  body.append(
    'insuranceNo',
    data.insuranceNumber && data.insuranceNumber.trim(),
  );
  body.append('insuranceExp', data.insuranceExpiryDate);
  body.append(
    'equipment',
    data.selectedItems ? JSON.stringify(data.selectedItems) : null,
  );
...
export const userDriverSecondMediateForm = (data, saveState, onSubmitform) => {
  ...
  body.append(
    'plateNo',
    !data.vehicleNumber ? '' : data.vehicleNumber && data.vehicleNumber.trim(),
  );
  ...
  body.append(
    'insuranceExp',
    !data.insuranceExpiryDate ? '' : data.insuranceExpiryDate,
  );
  body.append(
    'equipment',
    data.selectedItems ? JSON.stringify(data.selectedItems) : null,
  ...
  return dispatch => {
    return new Promise(function(fulfill, reject) {
      AsyncStorage.getItem('token').then(tokenValue => {
        dispatch(startLoading());
        RestClient.imageUpload('users/savevehicleinfo', body, tokenValue)
          .then(result => {
            console.log('result vehicle info ******* ', result);
            fulfill(result);
            if (result.status == 1) {
...
export const getDriverData = token => {
  return dispatch => {
    let userToken = AsyncStorage.getItem('token');
    dispatch(startLoading());
    RestClient.post('users/getDriver', {}, userToken)
      .then(result => {
...
export const getDriverStatus = token => {
  return dispatch => {
    dispatch(startLoading());
    RestClient.post('users/getDriver', {}, token)
      .then(result => {
        console.log('result getDriverData ****** ', result);
        if (result.status === 1) {
          dispatch(stopLoading());
...
export const logout = data => {
  ...
  return dispatch => {
    dispatch(startLoading());
    RestClient.post('users/logout', {}, data)
      .then(result => {
...
export const resendForgotApi = data => {
  ...
  return dispatch => {
    dispatch(startLoading());
    RestClient.post('users/resend', requestObject)
      .then(result => {
        //console.log('result email verify ******* ',result)
        if (result.status == 1) {
          dispatch(stopLoading());
...
export const availibilityStatus = (data, token) => {
  ...
  return dispatch => {
    dispatch(startLoading());
    RestClient.post('drivers/setAvailablity', requestObject, token)
      .then(result => {
...
export const getCertificatesList = token => {
  return dispatch => {
    dispatch(startLoading());
    RestClient.get('drivers/getCertificate', {}, token)
      .then(result => {
...
export const get_order_History = (url, data) => {
  return dispatch => {
    return new Promise(function(fulfill, reject) {
      dispatch(startLoading());
      RestClient.get_New(
        '/place-order/order-history?driverId=5ba512c5405cff32dcd952f4',
        {},
        '',
      )
        .then(result => {
...
export const acceptOrder = data => {
  return dispatch => {
    return new Promise(function(fulfill, reject) {
      ...
      RestClient.get_New_Patch('/place-order/accept', data, '')
        .then(result => {
          if (result.status == 1) {
...
export const rejectOrder = data => {
  return dispatch => {
    return new Promise(function(fulfill, reject) {
      dispatch(startLoading());
      RestClient.get_New_Patch('/place-order/reject-order', data, '')
        .then(result => {
          if (result.status == 1) {
            /*dispatch({
						type: 'SET_ACCEPTED_OREDERDATA',
...
export const orderPicked = data => {
  return dispatch => {
    return new Promise(function(fulfill, reject) {
      dispatch(startLoading());
      RestClient.get_New_Patch('/place-order/pickup/', data, '')
        .then(result => {
          if (result.status == 1) {
...
export const markArrive = data => {
  return dispatch => {
    return new Promise(function(fulfill, reject) {
      dispatch(startLoading());
      RestClient.get_New_Patch('/place-order/arrive/', data, '')
        .then(result => {
          if (result.status == 1) {
            //dispatch({type:'SET_START_ORDERDATA',data:result.data});
            //dispatch({type:'NEXT_PICKUP',data:result.data});
...
export const dropArrive = data => {
  return dispatch => {
    return new Promise(function(fulfill, reject) {
      dispatch(startLoading());
      RestClient.get_New_Patch('/place-order/dropup-arrive/', data, '')
        .then(result => {
          if (result.status == 1) {
            fulfill(result.data);
            dispatch(stopLoading());
...
export const dropDone = data => {
  return dispatch => {
    return new Promise(function(fulfill, reject) {
      dispatch(startLoading());
      RestClient.get_New_Patch('/place-order/dropup/', data, '')
        .then(result => {
          if (result.status == 1) {
            fulfill(result.data);
            dispatch(stopLoading());
...
export const nextStop = data => {
  return dispatch => {
    return new Promise(function(fulfill, reject) {
      dispatch(startLoading());
      RestClient.get_New_Patch('/place-order/start-order/', data, '')
        .then(result => {
          if (result.status == 1) {
            fulfill(result);
            dispatch(stopLoading());
...
export const chnageOorderPriority = data => {
  return dispatch => {
    return new Promise(function(fulfill, reject) {
      dispatch(startLoading());
      RestClient.get_New_Patch('/place-order/pickup-priority', data, '')
        .then(result => {
          if (result.status == 1) {
            fulfill(result.data);
            dispatch(stopLoading());
...
export const Ordered_details = data => {
  return dispatch => {
    return new Promise(function(fulfill, reject) {
      dispatch(startLoading());
      RestClient.get_New_Patch('place-order/orderDetails/' + data)
        .then(result => {
